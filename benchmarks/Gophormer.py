...
def eval_train(args, model, device, loader):
    ...
    t1 = time.time()
    sum_time += t1-t0
    timer.add_time('eval_train',sum_time)
...
def eval(args, model, device, loader):
    ...
    t1 = time.time()
    sum_time += t1-t0
    timer.add_time('eval',sum_time*2)
...
#     def __init__(self, N):
#         self.attn_bias = [None for i in range(N)]
#         self.feature_id = [None for i in range(N)]
#         self.label = [None for i in range(N)]

#     def __len__(self):
#         return len(self.attn_bias)

#     def __getitem__(self, idx):
#         return (self.data[idx], self.feature_id[idx], self.label[idx])

#     def sample_data(self, data, loader, power_adj_list):
#         s0 = 0
#         s1 = 0
#         s2 = 0
#         print(power_adj_list[0].edge_index)
#         for i, batch in tqdm(enumerate(loader)):
#             node_feature_id = batch.n_id
#             num_nodes = len(node_feature_id)
            
#             t0 = time.time()
            
#             pal = [i.subgraph(node_feature_id.long()) for i in power_adj_list]

#             t1 = time.time()

#             pal = [torch.sparse_coo_tensor(torch.stack([sg.edge_index[0], sg.edge_index[1]], dim=1).t(), sg.edge_attr, (num_nodes, num_nodes)) for sg in pal]
...
#             t2 = time.time()
#             self.attn_bias[i] = torch.stack(pal, dim=2).to_dense()
...
#             self.label[i] = data.y[node_feature_id].long()
#             self.feature_id[i] = node_feature_id
...
def main():
    ...
    print(data)
    y = data.y
    power_adj_list = torch.load('./dataset/' + args.dataset_name + '/pal.pt')
    power_adj_list['N'].n_id = torch.arange(data.num_nodes)
    ...
    fanout = [25,10]
    fake_fanout = [0] * len(fanout)
    num_neighbors = {}
    num_neighbors['N','E','N'] = fanout
    for i in range(len(power_adj_list.edge_types)-1):
        num_neighbors['N',f'A^{i}','N'] = fake_fanout

        
    from torch_geometric.loader import NeighborLoader
    collator = partial(Gophormer_collator, enc_dim = args.attn_bias_dim, device=device)
    train_loader = DataLoader(WrapperLoader(NeighborLoader(
            power_adj_list,
            input_nodes=('N', train_mask),
            subgraph_type='induced',
            num_neighbors=num_neighbors,  # 两层采样，分别采样25和10个邻居
            batch_size=1,             # 每个子图一个中心节点
            shuffle=True,            # 保持原始顺序
        )), batch_size=args.batch_size, collate_fn=collator
    )
    test_loader = DataLoader(WrapperLoader(NeighborLoader(
            power_adj_list,
            input_nodes=('N', test_mask),
            subgraph_type='induced',
            num_neighbors=num_neighbors,  # 两层采样，分别采样25和10个邻居
            batch_size=1,             # 每个子图一个中心节点
    ...
    val_loader = DataLoader(WrapperLoader(NeighborLoader(
            power_adj_list,
            input_nodes=('N', valid_mask),
            subgraph_type='induced',
            num_neighbors=num_neighbors,  # 两层采样，分别采样25和10个邻居
            batch_size=1,             # 每个子图一个中心节点
            shuffle=False,            # 保持原始顺序
