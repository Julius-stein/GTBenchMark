...
from models.GraphGPS import GPSModel
...
def main():
    ...
    net_params['attn_type'] = 'graphattn'
    ...
    collator = partial(GraphTransformer_collator,
                       pos_enc_dim=net_params['pos_enc_dim'],
                       lap_pos_enc=net_params['lap_pos_enc'],
                       wl_pos_enc=net_params['wl_pos_enc'])
    ...
    prof = torch.profiler.profile(
        ...
        schedule=torch.profiler.schedule(wait=1, warmup=0, active=1, repeat=10),
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./log/'+'GraphGPS'),
    ...
    for epoch in range(1, args.epochs+1):
        ...
        with timer.timer('all_train'):
            train(args, model, device, train_loader, optimizer, lr_scheduler, prof=prof)
            lr_scheduler.step()
