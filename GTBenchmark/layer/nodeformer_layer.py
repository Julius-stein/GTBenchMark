...
class NodeFormerConv(nn.Module):
    ...
    #              nb_random_features=10, use_gumbel=True, nb_gumbel_sample=10, rb_order=0, rb_trans='sigmoid', use_edge_loss=True):
        ...
        self.Wo = nn.Linear(dim_h * self.n_heads, dim_h)
        if cfg.gt.rb_order >= 1:
            self.b = torch.nn.Parameter(torch.FloatTensor(cfg.gt.rb_order, self.n_heads), requires_grad=True)

        if cfg.gt.rb_order >= 1:
            if cfg.gt.rb_trans == 'sigmoid':
                torch.nn.init.constant_(self.b, 0.1)
            elif cfg.gt.rb_trans == 'identity':
                torch.nn.init.constant_(self.b, 1.0)
        
        self.dim_out = dim_h
        if cfg.gt.kernel_trans == 'softmax':
            self.kernel_transformation = softmax_kernel_transformation
        elif cfg.gt.kernel_trans == 'relu':
        ...
        self.projection_matrix_type = cfg.gt.projection_matrix_type
        self.nb_random_features = cfg.gt.nb_random_features
        self.use_gumbel = cfg.gt.use_gumbel
        self.nb_gumbel_sample = cfg.gt.nb_gumbel_sample
        self.rb_order = cfg.gt.rb_order
        self.rb_trans = cfg.gt.rb_trans
        self.use_edge_loss = cfg.gt.use_edge_loss
        self.tau = cfg.gt.tau
        self.use_residual = cfg.gt.residual
        self.use_bn = cfg.gt.batch_norm
        self.use_act = cfg.gt.use_act
