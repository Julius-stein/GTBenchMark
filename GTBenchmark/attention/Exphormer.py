...
class ExphormerAttention(nn.Module):
    def __init__(self, dim_h, num_heads, attn_drop=0.0,dim_edge=None,return_attn_weights: bool = False):
        super().__init__()
        ...
        if cfg.dataset.num_virt_node >0:
            self.use_virt_nodes = True
        else:
            self.use_virt_nodes = False
        self.use_bias = False

        if dim_edge is None:
            dim_edge = dim_h

        self.Q = nn.Linear(dim_h, self.out_dim * num_heads, bias=self.use_bias)
        self.K = nn.Linear(dim_h, self.out_dim * num_heads, bias=self.use_bias)
        self.E = nn.Linear(dim_edge, self.out_dim * num_heads, bias=self.use_bias)
        self.V = nn.Linear(dim_h, self.out_dim * num_heads, bias=self.use_bias)
