...
class MultiHeadGraphAttentionLayer(MessagePassing):
    def __init__(self, in_dim:int, out_dim:int, n_heads:int, dropout, bias, edge_dim=None, use_edges=False):
        ...
        self.WQ = nn.Linear(in_dim, out_dim, bias=self.use_bias)
        self.WK = nn.Linear(in_dim, out_dim, bias=self.use_bias)
        self.WV = nn.Linear(in_dim, out_dim, bias=self.use_bias)
        if edge_dim is not None:
            self.WE = nn.Linear(in_dim, out_dim, bias=self.use_bias)
    ...
    def forward(self, x, edge_index, edge_attr=None):
        ...
        if self.edge_dim is not None:
            ...
            E = E_h.view(-1, self.n_heads, self.head_dim)
        else:
            E = None
        h = self.propagate(edge_index=edge_index, Q=Q, K=K, V=V).view(-1, self.out_dim)
        return h, None
    
    def message(self, Q_i, K_j, V_j, index, edge_bias=None):
        d_k = Q_i.size(-1)
...
class GraphTransformerLayer(MessagePassing):
    def __init__(self, in_dim, hidden_dim:int=128, edge_dim=None, n_heads:int=8, dropout:float=0.0, aggregators: List[str]=['sum'], use_edges=False, batch_norm=False):
        ...
        self.attention = MultiHeadGraphAttentionLayer(in_dim=in_dim, out_dim=hidden_dim // n_heads, edge_dim=self.edge_dim, n_heads=n_heads, use_edges=self.use_edges)
